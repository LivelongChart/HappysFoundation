...
document.querySelectorAll('[data-service-select]').forEach(serviceLink=>serviceLink.addEventListener('click',event=>{const requestSection=document.querySelector('#request');const serviceSelect=requestSection?.querySelector('select[name="service"]');if(!requestSection||!serviceSelect)return;event.preventDefault();const serviceName=serviceLink.dataset.serviceSelect;if([...serviceSelect.options].some(option=>option.value===serviceName)){serviceSelect.value=serviceName;serviceSelect.dispatchEvent(new Event('change',{bubbles:true}))}requestSection.scrollIntoView({behavior:reduced?'auto':'smooth',block:'start'});serviceSelect.focus({preventScroll:true});history.replaceState(null,'','#request')}));
